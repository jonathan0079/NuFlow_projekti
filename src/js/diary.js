...
function setupModalFunctionality() {
  ...
  if (chartOpen) {
    chartOpen.addEventListener('click', async () => {
      ...
      try {
        const user = JSON.parse(localStorage.getItem('user'));
        const res = await fetch('http://72.145.13.207:5000/api/kubios/hrv/last-30-measurements', {
          headers: {
  ...
  if (btn7) {
    btn7.addEventListener('click', async () => {
      ...
      try {
        const user = JSON.parse(localStorage.getItem('user'));
        const res = await fetch('http://72.145.13.207:5000/api/kubios/hrv/last-7-measurements', {
          headers: {
            'Authorization': `Bearer ${user.token}`
          }
        });
...
function initDiary(token) {
  ...
  const API_URL = 'http://72.145.13.207:5000/api/entries/insert'; // Backend-osoite merkintöjen lisäykseen
...
async function fetchAndDisplayHrvData(token) {
  ...
  try {
    const response = await fetch(`http://72.145.13.207:5000/api/kubios/hrv/by-date/${staticDate}`, {
      headers: {
...
async function fetchMonthHrvData(token) {
  try {
    ...
    async function fetchHrvDataForSelectedDate(token, date) {
      ...
      try {
        const response = await fetch(`http://72.145.13.207:5000/api/kubios/hrv/by-date/${date}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
